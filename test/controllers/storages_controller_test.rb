...
class StoragesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create storage" do
    assert_difference("Storage.count") do
      post storages_url, params: { storage: { block: @storage.block, stornum: @storage.stornum } }
    end
  ...
  test "should update storage" do
    patch storage_url(@storage), params: { storage: { block: @storage.block, stornum: @storage.stornum } }
